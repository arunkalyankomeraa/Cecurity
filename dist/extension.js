...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.calculateSecurityScore = exports.analyzeCode = void 0;
const securityRules = [
    // SQL Injection Rules
    {
...
function analyzeSecurityPatterns(code, issues) {
...
function calculateSecurityScore(highIssues, mediumIssues) {
    const baseScore = 100;
...
const vscode = __webpack_require__(/*! vscode */ "vscode");
const aiAnalyzer_1 = __webpack_require__(/*! ./aiAnalyzer */ "./src/aiAnalyzer.ts");
class SecurityPanel {
    ...
    show() {
        if (this.panel) {
            ...
            this.panel = vscode.window.createWebviewPanel(SecurityPanel.viewType, 'Security Report', vscode.ViewColumn.Beside, {
                enableScripts: true,
    ...
    update(issues) {
        ...
        this.panel.webview.html = this.getHtmlContent(score, highIssues, mediumIssues, lowIssues);
    }
    ...
    getHtmlContent(score, highIssues, mediumIssues, lowIssues) {
        return `
            <!DOCTYPE html>
            <html>
            <head>
...
SecurityPanel.viewType = 'securityReport';
...
class StatusBarManager {
    constructor() {
        ...
        this.issuesStatusBarItem = vscode.window.createStatusBarItem(vscode.StatusBarAlignment.Right, 99);
    ...
    getScoreTooltip(score) {
        if (score >= 90)
            return 'Excellent Security Score';
        if (score >= 70)
            return 'Good Security Score';
        if (score >= 50)
            return 'Fair Security Score';
...
const vector_stores_1 = __webpack_require__(/*! ./vector-stores/vector-stores.js */ "./node_modules/openai/resources/beta/vector-stores/vector-stores.js");
...
const file_batches_1 = __webpack_require__(/*! ./file-batches.js */ "./node_modules/openai/resources/beta/vector-stores/file-batches.js");
...
const securityPanel_1 = __webpack_require__(/*! ./securityPanel */ "./src/securityPanel.ts");
...
function activate(context) {
    ...
    securityPanel = new securityPanel_1.SecurityPanel(context.extensionUri);
    ...
    config.update('openAIKey', apiKey, true).then(() => {
        aiAnalyzer = new aiAnalyzer_1.AIAnalyzer();
        vscode.window.showInformationMessage('OpenAI API Key has been configured');
    });
    // Register commands
    context.subscriptions.push(vscode.commands.registerCommand('security-analyzer.analyze', () => {
        if (activeEditor) {
            analyzeCurrentFile(activeEditor, diagnosticCollection);
        }
    }));
    context.subscriptions.push(vscode.commands.registerCommand('security-analyzer.showReport', () => {
        securityPanel.show();
    }));
    context.subscriptions.push(vscode.commands.registerCommand('security-analyzer.aiAnalyze', async () => {
        if (activeEditor) {
            await analyzeWithAI(activeEditor, diagnosticCollection);
        }
    }));
    context.subscriptions.push(vscode.commands.registerCommand('security-analyzer.setOpenAIKey', async () => {
        ...
        if (key) {
            ...
            aiAnalyzer = new aiAnalyzer_1.AIAnalyzer(); // Reinitialize with new key
...
async function analyzeWithAI(editor, diagnosticCollection) {
    try {
        const text = editor.document.getText();
        const aiIssues = await aiAnalyzer.analyzeCode(text);
        const standardIssues = (0, analyzer_1.analyzeCode)(text);
...
function analyzeCurrentFile(editor, diagnosticCollection) {
    const text = editor.document.getText();
    const issues = (0, analyzer_1.analyzeCode)(text);
...
function updateDiagnostics(editor, diagnosticCollection, issues) {
    const diagnostics = issues.map(issue => {
        const range = new vscode.Range(issue.line - 1, 0, issue.line - 1, 100);
        const diagnostic = new vscode.Diagnostic(range, issue.message, convertSeverity(issue.severity));
...
function updateSecurityScore(issues) {
    const highIssues = issues.filter(i => i.severity === 'high').length;
    const mediumIssues = issues.filter(i => i.severity === 'medium').length;
    const score = (0, analyzer_1.calculateSecurityScore)(highIssues, mediumIssues);
